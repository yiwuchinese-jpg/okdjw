...
export default async function ArticleDetailPage({
  ...
  if (sanityData) {
    ...
    const jsonLd = {
      ...
      "@type": "BlogPosting",
      "headline": sanityData.title,
      "description": sanityData.description,
      "image": sanityData.image ? [sanityData.image] : [],
      "datePublished": sanityData.date,
      "author": [{
        "@type": "Person",
        "name": "Justin Du",
        "url": "https://okdjw.com"
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <header className="mb-16">
            <div className="flex flex-wrap gap-4 mb-8">
              {sanityData.tags?.map((tag: string) => (
              ...
              <span className="flex items-center gap-2 px-4 py-1 bg-white/5 text-white/40 border border-white/10 rounded-full text-[10px] font-bold uppercase tracking-widest">
                <Calendar className="w-3 h-3" /> {new Date(sanityData.date).toLocaleDateString()}
              </span>
            </div>
