...
export async function generateMetadata({
  ...
  try {
    // 1. Try Sanity first
    const sanityData = await getSanityContentData('post', slug, locale); // Tutorials usually map to posts in Sanity if exists
    if (sanityData) {
      return {
    ...
    const data = await getContentData("tutorials", slug, locale);
    return {
      title: data.title,
